...
import { PencilIcon, Trash2Icon, PlusIcon, PiggyBankIcon, LockIcon, UsersIcon, ClipboardListIcon, DollarSignIcon, ArrowUpIcon, ArrowDownIcon, CreditCardIcon, FileTextIcon, CalendarIcon, TrendingUpIcon, TrendingDownIcon, BarChart2Icon, DownloadIcon, CashIcon, StarIcon, LightbulbIcon, TargetIcon, PrinterIcon } from '../constants';
import { transactionsService, financialPocketsService, cardsService } from '../services/supabaseService';
...
const CashWidget: React.FC<{ card: Card, onTopUp: () => void, onEdit: () => void, onClick: () => void }> = ({ card, onTopUp, onEdit, onClick }) => (
    <div className="group relative cursor-pointer" onClick={onClick}>
        <div className={`p-6 rounded-2xl text-gray-800 shadow-lg flex flex-col justify-between aspect-[1.586] bg-gradient-to-br ${card.colorGradient}`}>
            <div>
                <div className="flex justify-between items-center">
            ...
            <p className="text-sm text-gray-600">Akun untuk mencatat transaksi tunai.</p>
        </div>
        <div className="absolute inset-0 bg-black/50 rounded-2xl opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 non-printable">
...
const Finance: React.FC<FinanceProps> = ({ transactions, setTransactions, pockets, setPockets, projects, profile, cards, setCards, teamMembers, rewardLedgerEntries }) => {
    ...
    const handleCloseBudget = async (budgetPocket: FinancialPocket, isAutomatic: boolean = false) => {
        ...
        try {
            if (budgetPocket.amount > 0) {
                const newSavedPocket: Omit<FinancialPocket, 'id'> = {
                ...
                await financialPocketsService.create(newSavedPocket);

                const closingTx: Omit<Transaction, 'id'> = {
                ...
                await transactionsService.create(closingTx);

                await financialPocketsService.update(budgetPocket.id, { amount: 0, name: currentMonthName });

                showNotification(`Anggaran "${closedPocketName}" ditutup. Sisa ${formatCurrency(budgetPocket.amount)} disimpan.`);
            } else {
                await financialPocketsService.update(budgetPocket.id, { name: currentMonthName });
                if (isAutomatic) {
            ...
            showNotification(`Gagal menutup anggaran: ${error.message}`);
        }
    };
    ...
    const cardStats = useMemo(() => {
        const creditDebt = cards
            ...
            .reduce((sum, c) => sum + c.balance, 0); // balance is negative
        ...
        const cashBalance = cards
            .find(c => c.cardType === CardType.TUNAI && c.bankName === 'Tunai')?.balance || 0;
        ...
        const mostUsedCardId = Object.keys(transactionCounts).length > 0
            ? Object.keys(transactionCounts).sort((a, b) => transactionCounts[b] - transactionCounts[a])[0]
            : null;
            
        const mostUsedCard = mostUsedCardId ? cards.find(c => c.id === mostUsedCardId) : null;
        const mostUsedCardName = mostUsedCard ? `${mostUsedCard.bankName} (...${mostUsedCard.lastFourDigits})` : 'N/A';
        const mostUsedCardTxCount = mostUsedCardId ? transactionCounts[mostUsedCardId] : 0;
        
        return { creditDebt, debitAndCashAssets, cashBalance, mostUsedCardName, mostUsedCardTxCount };
    }, [cards, transactions]);
    ...
    const expenseDonutData = useMemo(() => {
        ...
        const colors = ['#f87171', '#fb923c', '#facc15', '#a3e635', '#34d399', '#22d3ee', '#60a5fa', '#a78bfa', '#f472b6'];
            return Object.entries(expenseByCategory)
                ...
                .map(([label, value], i) => ({ label, value: Number(value) || 0, color: colors[i % colors.length] }));
    }, [transactions]);
    ...
    const renderTabContent = () => {
        switch (activeTab) {
            case 'transactions':
                ...
                const incomeVals = Object.values(categoryTotals.income).map(v => Number(v) || 0);
                const expenseVals = Object.values(categoryTotals.expense).map(v => Number(v) || 0);
                const allIncomeTotal = incomeVals.reduce((sum, amount) => sum + amount, 0);
                const allExpenseTotal = expenseVals.reduce((sum, amount) => sum + amount, 0);
                        ...
                        <div className="lg:col-span-1 space-y-6">
                            ...
                            <div className="bg-brand-surface p-4 rounded-2xl shadow-lg border border-brand-border">
                                ...
                                <div className="space-y-1">
                                    ...
                                    {Object.entries(categoryTotals.income).map(([name, amount]) => {
                                        const numAmount = Number(amount) || 0;
                                        return (<React.Fragment key={name}><CategoryButton type={TransactionType.INCOME} categoryName={name} amount={numAmount} isActive={categoryFilter.type === TransactionType.INCOME && categoryFilter.category === name} onClick={() => setCategoryFilter({ type: TransactionType.INCOME, category: name })} /></React.Fragment>)
                                    })}
                                </div>
                            </div>
                            <div className="bg-brand-surface p-4 rounded-2xl shadow-lg border border-brand-border">
                                <h4 className="font-semibold text-gradient mb-3 px-2">Pengeluaran</h4>
                                 <div className="space-y-1">
                                    ...
                                    {Object.entries(categoryTotals.expense).map(([name, amount]) => {
                                        const numAmount = Number(amount) || 0;
                                        return (<React.Fragment key={name}><CategoryButton type={TransactionType.EXPENSE} categoryName={name} amount={numAmount} isActive={categoryFilter.type === TransactionType.EXPENSE && categoryFilter.category === name} onClick={() => setCategoryFilter({ type: TransactionType.EXPENSE, category: name })} /></React.Fragment>)
                                    })}
                                </div>
                            </div>
                        </div>
            ...
            case 'labaProyek':
                ...
                const yearOptions = Array.from(new Set(projects.map(p => Number(new Date(p.date).getFullYear())))).map(y => Number(y)).sort((a,b) => b-a);
        ...
        <div className="space-y-6">
            <PageHeader title="Manajemen Keuangan" subtitle="Pantau kesehatan finansial bisnis Anda dari transaksi hingga proyeksi masa depan.">
                <div className="flex items-center gap-2 non-printable">
                    ...
                    <button onClick={() => handleOpenModal('transaction', 'add')} className="button-primary inline-flex items-center gap-2">
            ...
            <Modal isOpen={isInfoModalOpen} onClose={() => setIsInfoModalOpen(false)} title="Panduan Halaman Keuangan">
                <div className="space-y-4 text-sm text-brand-text-primary">
                    <p>Halaman ini adalah pusat komando finansial bisnis Anda. Gunakan berbagai tab untuk mendapatkan gambaran lengkap.</p>
                    <ul className="list-disc list-inside space-y-2">
                        ...
                        <li><strong>Laporan:</strong> Hasilkan laporan keuangan profesional. Anda bisa memfilter berdasarkan klien dan rentang tanggal, lalu mencetaknya sebagai PDF.</li>
                    </ul>
                </div>
            </Modal>
