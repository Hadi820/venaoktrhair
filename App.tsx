...
import { clientsService, projectsService, leadsService, transactionsService,
  teamMembersService, teamProjectPaymentsService, financialPocketsService,
  cardsService, assetsService, contractsService, clientFeedbackService, socialMediaPostsService,
  promoCodesService, sopsService, packagesService, addOnsService, notificationsService
} from './services/supabaseService';
import { teamPaymentRecordsService, rewardLedgerEntriesService } from './services/supabaseService';
...
const App: React.FC = () => {
  ...
  const teamProjectPaymentsRef = useRef(teamProjectPayments);
   const teamPaymentRecordsRef = useRef(teamPaymentRecords);
  const pocketsRef = useRef(pockets);
  const rewardLedgerEntriesRef = useRef(rewardLedgerEntries);
  const cardsRef = useRef(cards);
  const assetsRef = useRef(assets);
  const contractsRef = useRef(contracts);
  const clientFeedbackRef = useRef(clientFeedback);
  // teamPaymentRecordsRef intentionally omitted
  const socialMediaPostsRef = useRef(socialMediaPosts);
  ...
  useEffect(() => { teamProjectPaymentsRef.current = teamProjectPayments }, [teamProjectPayments]);
   useEffect(() => { teamPaymentRecordsRef.current = teamPaymentRecords }, [teamPaymentRecords]);
  useEffect(() => { pocketsRef.current = pockets }, [pockets]);
  useEffect(() => { rewardLedgerEntriesRef.current = rewardLedgerEntries }, [rewardLedgerEntries]);
  useEffect(() => { cardsRef.current = cards }, [cards]);
  useEffect(() => { assetsRef.current = assets }, [assets]);
  useEffect(() => { contractsRef.current = contracts }, [contracts]);
  useEffect(() => { clientFeedbackRef.current = clientFeedback }, [clientFeedback]);
  // teamPaymentRecordsRef intentionally omitted
  useEffect(() => { socialMediaPostsRef.current = socialMediaPosts }, [socialMediaPosts]);
  useEffect(() => { promoCodesRef.current = promoCodes }, [promoCodes]);
  ...
  // Helper to compute diffs and sync
  const computeDiffsAndSync = async <T extends { id?: string }>(
    prev: T[],
    next: T[],
    service: any,
    ...
    // Detect created (id not in prev)
  for (const n of next) {
      if (!n.id || !prevById.has(n.id)) {
        try {
          const payload = { ...n };
          delete (payload as any).id;
          const created = await service.create(payload);
          // replace temporary item (match by some unique fields or by absence of prev) - we'll match by index fallback
          setState(prevState => prevState.map(it => (it === n || it.id === n.id) ? created : it));
      showNotification && showNotification(`${entityName} tersimpan di Supabase`);
        } catch (err: any) {
          console.error(`Error creating ${entityName}:`, err);
      showNotification && showNotification(`Gagal menyimpan ${entityName} ke Supabase: ${err?.message || err}`);
        }
      }
    }
      ...
      if (n.id && prevById.has(n.id)) {
        ...
        try {
          if (JSON.stringify(p) !== JSON.stringify(n)) {
            ...
            const updated = await service.update(n.id, updates);
            setState(prevState => prevState.map(it => (it.id === n.id ? updated : it)));
            showNotification && showNotification(`${entityName} diupdate di Supabase`);
          }
        } catch (err: any) {
          console.error(`Error updating ${entityName}:`, err);
          showNotification && showNotification(`Gagal update ${entityName} di Supabase: ${err?.message || err}`);
        }
      }
    }
    ...
    for (const p of prev) {
      if (p.id && !nextById.has(p.id)) {
        try {
          ...
          showNotification && showNotification(`${entityName} dihapus di Supabase`);
        } catch (err: any) {
          console.error(`Error deleting ${entityName}:`, err);
          showNotification && showNotification(`Gagal menghapus ${entityName} di Supabase: ${err?.message || err}`);
        }
      }
    }
  };

  // Generic sync setter creator
  const makeSyncSetter = <T extends { id?: string }>(
    ref: React.RefObject<T[]>,
    origSet: React.Dispatch<React.SetStateAction<T[]>>,
    service: any,
    entityName = 'item'
  ) => {
    return (action: React.SetStateAction<T[]>) => {
      ...
      origSet(next as T[]);
      // Sync in background
      computeDiffsAndSync(prev, next as T[], service, origSet, entityName).catch(err => {
        console.error('Background sync error for', entityName, err);
      });
    };
  };

  const setClientsSync = makeSyncSetter<Client>(clientsRef, setClients, clientsService, 'client');
  const setProjectsSync = makeSyncSetter<Project>(projectsRef, setProjects, projectsService, 'project');
  const setLeadsSync = makeSyncSetter<Lead>(leadsRef, setLeads, leadsService, 'lead');
  const setTransactionsSync = makeSyncSetter<Transaction>(transactionsRef, setTransactions, transactionsService, 'transaction');
  const setTeamMembersSync = makeSyncSetter<TeamMember>(teamMembersRef, setTeamMembers, teamMembersService, 'team member');
  const setTeamProjectPaymentsSync = makeSyncSetter<TeamProjectPayment>(teamProjectPaymentsRef, setTeamProjectPayments, teamProjectPaymentsService, 'team project payment');
  const setTeamPaymentRecordsSync = makeSyncSetter<TeamPaymentRecord>(teamPaymentRecordsRef, setTeamPaymentRecords, teamPaymentRecordsService, 'team payment record');
  const setPocketsSync = makeSyncSetter<FinancialPocket>(pocketsRef, setPockets, financialPocketsService, 'pocket');
  const setRewardLedgerEntriesSync = makeSyncSetter<RewardLedgerEntry>(rewardLedgerEntriesRef, setRewardLedgerEntries, rewardLedgerEntriesService, 'reward ledger entry');
  const setCardsSync = makeSyncSetter<Card>(cardsRef, setCards, cardsService, 'card');
  const setAssetsSync = makeSyncSetter<Asset>(assetsRef, setAssets, assetsService, 'asset');
  const setContractsSync = makeSyncSetter<Contract>(contractsRef, setContracts, contractsService, 'contract');
  const setClientFeedbackSync = makeSyncSetter<ClientFeedback>(clientFeedbackRef, setClientFeedback, clientFeedbackService, 'client feedback');
  // sync setters for teamPaymentRecords intentionally omitted
  const setSocialMediaPostsSync = makeSyncSetter<SocialMediaPost>(socialMediaPostsRef, setSocialMediaPosts, socialMediaPostsService, 'social post');
  const setPromoCodesSync = makeSyncSetter<PromoCode>(promoCodesRef, setPromoCodes, promoCodesService, 'promo code');
  const setSopsSync = makeSyncSetter<SOP>(sopsRef, setSops, sopsService, 'sop');
  const setPackagesSync = makeSyncSetter<Package>(packagesRef, setPackages, packagesService, 'package');
  const setAddOnsSync = makeSyncSetter<AddOn>(addOnsRef, setAddOns, addOnsService, 'add-on');
  const setNotificationsSync = makeSyncSetter<Notification>(notificationsRef, setNotifications, notificationsService, 'notification');
  ...
  const renderView = () => {
    ...
    switch (activeView) {
      ...
      case ViewType.PROSPEK:
        return <Leads
            leads={leads} setLeads={setLeadsSync}
            clients={clients} setClients={setClientsSync}
            projects={projects} setProjects={setProjectsSync}
      ...
      case ViewType.PROJECTS:
        return <Projects 
          ...
          packages={packages}
          teamMembers={teamMembers}
          teamProjectPayments={teamProjectPayments} setTeamProjectPayments={setTeamProjectPaymentsSync}
          transactions={transactions} setTransactions={setTransactionsSync}
          initialAction={initialAction} setInitialAction={setInitialAction}
          profile={profile}
          showNotification={showNotification}
      ...
      case ViewType.TEAM:
        return (
          <Freelancers
            teamMembers={teamMembers}
            setTeamMembers={setTeamMembersSync}
            teamProjectPayments={teamProjectPayments}
            setTeamProjectPayments={setTeamProjectPaymentsSync}
            teamPaymentRecords={teamPaymentRecords}
            setTeamPaymentRecords={setTeamPaymentRecordsSync}
            transactions={transactions}
            setTransactions={setTransactionsSync}
            userProfile={profile}
            showNotification={showNotification}
            ...
            setRewardLedgerEntries={setRewardLedgerEntriesSync}
            pockets={pockets}
  ...
  return <Packages packages={packages} setPackages={setPackagesSync} addOns={addOns} setAddOns={setAddOnsSync} projects={projects} profile={profile} />;
      case ViewType.ASSETS:
  return <Assets assets={assets} setAssets={setAssetsSync} profile={profile} showNotification={showNotification} />;
      case ViewType.CONTRACTS:
    return <Contracts 
      contracts={contracts} setContracts={setContractsSync}
        ...
        />;
      case ViewType.SOP:
  return <SOPManagement sops={sops} setSops={setSopsSync} profile={profile} showNotification={showNotification} />;
    ...
    return <ClientReports 
      ...
      setFeedback={setClientFeedbackSync}
            showNotification={showNotification}
        />;
      case ViewType.SOCIAL_MEDIA_PLANNER:
  return <SocialPlanner posts={socialMediaPosts} setPosts={setSocialMediaPostsSync} projects={projects} showNotification={showNotification} />;
      case ViewType.PROMO_CODES:
  return <PromoCodes promoCodes={promoCodes} setPromoCodes={setPromoCodesSync} projects={projects} showNotification={showNotification} />;
      default:
        return <div />;
    }
  };
